...
<script>
  $(document).ready(function() {
            $('#sectionStud').DataTable({
                "ajax": {
                    "url": "sectionStudapi.php", // Your API endpoint
                    "type": "GET",
                    "data": function(d) {
                ...
                "columns": [
                    { "data": "UserID" }, // Display UserID directly
                    { 
                        "data": function(row) {
                            // Concatenate names, handling nulls with fallback
                            const fname = row.Fname || '';
                            const mname = row.Mname || ''; // Optional middle name
                            const lname = row.Lname || '';
                            return `${fname} ${mname} ${lname}`.trim(); // Trim excess spaces
                        }
                    },
                    { 
                        "data": function(row) {
                            // Display RoleName or fallback to a single space
                            return row.RoleName || ' ';
                        }
                    }
                ],
                "language": {
                    "emptyTable": "No students found for this section"
                },
                "paging": true,
                "searching": true,
                "info": true
...
<br>
    ...
    <form id="Createrole" method="POST" action="backend/groupingsapi.php" autocomplete="off">
...
try {
    $stmt = $pdo->query("SELECT RoleID, RoleName FROM roletbl where `Usertype` <> 3;");
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function addMemberDropdown() {
            ...
            memberDiv.innerHTML = `
                ...
                <input type="hidden" name="MemberRole" id="memberHidden${memberCount}" value="9">
